  ...
  return (
    <View>
      <View style={{ marginBottom: 24 }}>
        <Text style={{ color: "#f9fbff", fontSize: 32, fontWeight: "800" }}>History</Text>
      ...
      {loading && history.length === 0 ? (
        ...
        <View style={{ gap: 14 }}>
          {history.map((item) => (
            ...
            >
              <View style={{ width: 180, aspectRatio: 16 / 9, borderRadius: 16, overflow: "hidden", backgroundColor: colors.background.tertiary }}>
                <Image source={{ uri: item.thumbnailUrl }} style={{ width: "100%", height: "100%" }} contentFit="cover" />
              </View>
              <View style={{ flex: 1, marginLeft: 16 }}>
                <Text numberOfLines={1} style={{ color: "#f8fbff", fontSize: 17, fontWeight: "700" }}>
                  {item.title}
                </Text>
                <Text style={{ color: "rgba(255,255,255,0.56)", marginTop: 8 }}>{item.channelName}</Text>
