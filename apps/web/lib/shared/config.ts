...
export interface AppwriteConfig {
  ...
  appViewIncrementFunctionUrl?: string;
}
...
export function createAppwriteConfig(
  ...
  return {
    ...
    appViewIncrementFunctionUrl: env.APPWRITE_VIEW_INCREMENT_FUNCTION_URL || "",
  };
}
...
export function validateAppwriteConfig(config: AppwriteConfig): void {
  ...
  if (!config.naatsCollectionId) missingVars.push("APPWRITE_NAATS_COLLECTION_ID");
  if (!config.channelsCollectionId) missingVars.push("APPWRITE_CHANNELS_COLLECTION_ID");
